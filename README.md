 ROS2 ADAS Perception & Autonomous Driving Research Stack
...
The long-term objective is to evolve the stack from:
...
Each major algorithm is implemented as an independent ROS 2 node with explicit topic interfaces.
...
Where appropriate, the project implements both classical and learned approaches.
...
This makes computational and algorithmic trade-offs measurable.
...
Modern ADAS systems are not simply a single neural network.

A practical autonomous-driving system contains multiple interconnected layers:
...
The architecture is deliberately designed so that individual algorithms can be:
...
rather than hiding the complete pipeline behind a single end-to-end model.
