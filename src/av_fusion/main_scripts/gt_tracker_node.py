...
# =========================================================
# Kalman Tracker (Constant Velocity Model)
# =========================================================
class KalmanTracker:
...
class GTTracker(Node):
    ...
    def __init__(self):
        ...
        self.prev_ego_y = None
        self.prev_ego_time = None

        self.lane_width = 3.5

        self.get_logger().info("FINAL Production Tracker Started")
    ...
    def callback(self, msg):
        ...
        # ---------------- Ego velocity ----------------
        ego_x = transform.transform.translation.x
        ego_y = transform.transform.translation.y

        ego_vx = 0.0
        ego_vy = 0.0

        if self.prev_ego_x is not None:
            ego_dt = current_time - self.prev_ego_time
            if ego_dt > 0:
                ego_vx = (ego_x - self.prev_ego_x) / ego_dt
                ego_vy = (ego_y - self.prev_ego_y) / ego_dt

        self.prev_ego_x = ego_x
        self.prev_ego_y = ego_y
        self.prev_ego_time = current_time
        ...
        for trk in self.trackers:
            ...
            # Relative velocity
            rel_vx = vx_map - ego_vx
            rel_vy = vy_map - ego_vy

            vel_ego = R @ np.array([[rel_vx],[rel_vy]])
            v_long = vel_ego[0,0]
            ...
            start = Point(x=x_map, y=y_map, z=0.0)
            end = Point(x=x_map + vx_map, y=y_map + vy_map, z=0.0)
